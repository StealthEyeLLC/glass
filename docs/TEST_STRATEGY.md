...
| `bridge::tests::ws_live_session` | **`live_session_subscribe`** → `session_hello` (`f03_v0_live_ws`, `session_delta_wire_v0_server` when enabled) + `session_snapshot_replaced` after collector store change; optional **`session_delta`** after replacement when `BridgeConfig.session_delta_wire_v0` + subscribe `session_delta_wire: true`; capabilities `live_session_ingest` / `websocket.session_delta_wire_v0`; F-IPC multi-connection harness; frozen HTTP snapshot regression; **many short F-IPC connections → accept in a loop** |
