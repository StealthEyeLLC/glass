...
| `src/app/verticalSliceV0.test.ts` | **Vertical Slice v0** copy exports (`VERTICAL_SLICE_V0_ID`, hero subtitles; **v18** flagship session id `canonical_v15_append_heavy`; **v20** reading-order + receipt supplement strings) |
...
| `src/replay/verticalSliceV19AuditHardening.integration.test.ts` | **Vertical Slice v19** — mounted replay + flagship bytes: **temporal compare baseline** chip click clears bounded **claim** + **episode** selection and **primary-claim** chip highlight (trust handoff) |
...
| `src/live/liveSessionShell.test.ts` | **`live-vs-hero`**; **`live-reading-order`** (v20); mounted shell: **Disconnect** + **live-ws-status** + **live-session-log** + **live-visual-surface** / **canvas** / **live-visual-canvas-webgpu** / **live-visual-canvas-text-overlay** / **live-visual-gpu-status** / **fallback** / **live-visual-legend** / **live-visual-provenance-copy-json** / **live-visual-provenance-copy-text** test ids; **mock WebSocket**: operator close records **code** + **operator_disconnect**; **reconnect** clears prior close in JSON; **mock `navigator.clipboard`**: provenance **Copy JSON** success/failure → **session log** |
