...
| `src/scene/boundedSceneSelection.test.ts` | **Vertical Slice v5–v6** — stable **`glass.sel.v0:*`** ids; **`buildBoundedSelectionHitTargetsFromPrimitives`** / **`buildBoundedSelectionHitTargetsForScene`** + **`hitTestBoundedSelection`**; **`unionBoundingRectForSelectionId`**; **`buildBoundedInspectorLines`** (none / region / cluster) |
