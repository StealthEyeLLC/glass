...
| `bridge::tests::snapshot_fipc` | HTTP snapshot from collector over F-IPC: seeded `SnapshotStore`; **procfs fixture** per-RPC → `process_poll_sample`; **file-lane fixture** per-RPC → `file_poll_snapshot`; **retained** store tick + `retained_snapshot_unix_ms` on HTTP JSON; empty unknown session; capabilities `collector_fipc.configured` |
...
| `glass_collector::tests::ipc_fipc_tcp` | Provisional F-IPC TCP: handshake OK, wire/auth/secret mismatch rejections, bounded snapshot, listener smoke; **per-RPC procfs** JSON fixture: normalized snapshot, request + global caps, per-RPC JSON omits `retained_snapshot_unix_ms`; **per-RPC file-lane** fixture → `file_poll_snapshot`, empty `[]` honest cursor; **procfs + file-lane feeds** same runtime, distinct session ids; **retained store** path: `retained_snapshot_unix_ms` present, wrong-session meta omitted, per-RPC vs retained sessions independent |
...
| `glass-collector ipc-serve` | Loopback TCP F-IPC (provisional); optional seed session; **`--procfs-session`** = per-RPC procfs repoll; **`--file-lane-session`** + **`--file-lane-watch-root`** or **`--file-lane-from-raw-json`** = per-RPC file-lane (directory-poll semantics; **distinct** session id from procfs / retained); **`--procfs-retained-session`** + **`--procfs-retained-interval-ms`** / **`--procfs-retained-max-events`** = background bounded retained procfs store (same `--procfs-from-raw-json` / Linux rules); conflicting same session id across procfs / retained / file-lane rejected |
