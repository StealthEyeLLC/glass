...
| `bridge::tests::http_contract` | Loopback-only config; `/health` unauthenticated; bearer gate on `/capabilities` + snapshot; `collector_fipc` + **`websocket.live_session_delta_skeleton`** capability flags; bounded `SessionSnapshotResponse` without F-IPC (no `bounded_snapshot` / `max_events_requested`); **503** when F-IPC configured but collector unreachable; non-loopback F-IPC rejected; WS bad-request without upgrade; real WS handshake + hello JSON (`live_session_delta_skeleton` when F-IPC configured); `serve_listener` + `tokio-tungstenite` client |
| `bridge::tests::ws_live_session` | **`live_session_subscribe`** → `session_hello` (`f03_v0_live_ws` in global hello when F-IPC on) + `session_snapshot_replaced` after collector store change; F-IPC multi-connection harness; frozen HTTP snapshot contract regression; **integration tests that open many F-IPC connections must accept in a loop** (not single `accept()`), matching `run_ipc_dev_tcp_listener` |
| `glass_bridge::live_session_ws` (unit) | **`F03OutboundQueue`**: per-connection OR-threshold (**events** or **UTF-8 byte sum**); coalesce to latest `session_snapshot_replaced` + `session_resync_required` on overflow; mandatory resync on continuity paths; **`LIVE_WS_REASON_*` ≠** frozen HTTP **`RESYNC_HINT_REASON_*`**; see `docs/F03_LIVE_BACKLOG_FREEZE_PROPOSAL.md` |
