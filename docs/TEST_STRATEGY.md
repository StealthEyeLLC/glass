...
| `bridge::tests::snapshot_fipc` | HTTP snapshot from collector over F-IPC: seeded `SnapshotStore`; **procfs fixture** per-RPC → `process_poll_sample`; **retained** store tick + `retained_snapshot_unix_ms` on HTTP JSON; empty unknown session; capabilities `collector_fipc.configured` |
