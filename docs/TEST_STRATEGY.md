...
| `bridge::tests::snapshot_fipc` | HTTP snapshot from collector over F-IPC: seeded `SnapshotStore`; **procfs fixture** path → normalized `process_poll_sample` envelope; empty unknown session; capabilities `collector_fipc.configured` |
...
| `glass_collector::tests::ipc_fipc_tcp` | Provisional F-IPC TCP: handshake OK, wire/auth/secret mismatch rejections, bounded snapshot, listener smoke; **procfs `--from-raw-json`-equivalent** feed: non-empty normalized snapshot, request clamp, global `PROVISIONAL_FIPC_MAX_SNAPSHOT_EVENTS` clamp, mismatched session uses store only, empty raw array → `v0:empty` |
...
| `glass-collector ipc-serve` | Loopback TCP F-IPC (provisional); optional seed session; optional **`--procfs-session`** + **`--procfs-from-raw-json`** (non-Linux required) or Linux `/proc` for per-RPC bounded snapshots from real normalize path |
