...
| `src/replay/verticalSliceV31Overview.test.ts` | **Vertical Slice v31** — **Overview** idle **`overviewPhase`** + **`glass-overview-loaded-only`**; **Technical** retains **index-ordered** reading-order string; **primary actions** + helper; **DOM order** Scene → Evidence → Claim → Time → Episodes |
| `src/replay/verticalSliceV32PremiumVisual.test.ts` | **Vertical Slice v32** — **`glass-showcase-v32`** on replay + live roots; **Overview** default + **primary actions**; **Technical** toggle + **`glass-surface-technical-only`** unchanged |
...
| `src/app/devFixtureRoute.test.ts` | **`planDevFixtureLoad`**: **`vertical_slice_v0`** stays dev-only; **`flagship`** resolves in dev and static builds; smoke-only query stripping keeps the hosted flagship URL stable |
...
| `src/replay/firstRunEasyEntry.integration.test.ts` | **Vertical Slice v26** — **Start here** precedes live nav; primary live link label omits **`?live=`** / **`127.0.0.1`**; flagship **`<details>`** still contains **`?fixture=flagship`**; live shell — intro + collapsed connection; bridge input placeholder is not a literal loopback IP |
| `src/replay/verticalSliceV19AuditHardening.integration.test.ts` | **Vertical Slice v19** — mounted replay + flagship bytes: **temporal compare baseline** chip click clears bounded **claim** + **episode** selection and **primary-claim** chip highlight (trust handoff); **v21** asserts **empty-receipt supplement** text matches **`RECEIPT_EMPTY_SUPPLEMENT_AFTER_TEMPORAL_BASELINE`** (**Technical** surface — supplement is **not** shown on **Overview**) |
