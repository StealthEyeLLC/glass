...
| `src/scene/boundedClaims.test.ts` | **Vertical Slice v13–v14** — **`computeBoundedSceneClaims`**, **`resolvePrimaryClaimId`** (episode + cluster), append/replace status, **`buildBoundedClaimReceipt`** (**`glass.receipt.v0`**, ref keys, weakness), **`boundedClaimEvidenceUiLines`**, panel smoke |
