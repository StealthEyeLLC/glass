...
| `src/live/liveVisualWebGpu.test.ts` | **`hexToRgba01`**, **`pxRectToTriangleList`** quad size, **`buildLiveVisualWebGpuVertexData`** non-empty interleaved buffer; **`buildDrawablePrimitivesWebGpuVertexData`** matches scene-derived **`sceneToDrawablePrimitives`** for the same layout |
...
| `src/scene/drawablePrimitivesV0.test.ts` | **`buildBoundedVisualGeometryPrimitives`** stable ordering (background, band, **stroke** frame last); **`expandStrokeRectToFillRects`** |
| `src/scene/sceneToDrawablePrimitives.test.ts` | **`sceneToDrawablePrimitives`** equals **`buildBoundedVisualGeometryPrimitives(liveVisualSpecFromScene(scene), …)`**; explicit **layout** override |
