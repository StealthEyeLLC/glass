...
| `src/live/liveVisualMarkers.test.ts` | **`buildLiveVisualMarkersLayout`**: three fixed **slot** ticks (replace / append / resync wire) — **at most one active** from current `LiveVisualSpec.mode`; **HTTP** chip when `reconcileSummary` is set; **no** ticks active for hello / idle / none_delta / warning; deterministic **geometry**; **`liveVisualTickActiveFill`** |
...
| `src/live/liveSessionShell.test.ts` | Mounted shell: **Disconnect** + **live-ws-status** + **live-session-log** + **live-visual-surface** / **canvas** / **fallback** test ids; **mock WebSocket**: operator close records **code** + **operator_disconnect**; **reconnect** clears prior close in JSON |
