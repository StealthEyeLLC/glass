...
| `src/live/liveVisualCanvas.test.ts` | **`renderLiveVisualIntoContext`** with a pure mock 2D context (geometry + labels); **`renderLiveVisualOnCanvas`** integration (canvas sized; **`getContext("2d") === null`** returns `false`); Vitest **`viewer/vitest.setup.ts`** stubs jsdom’s missing Canvas 2D so the suite stays quiet |
