...
| `src/scene/sceneToLiveVisualSpec.test.ts` | Renderer boundary: replay scene → **`LiveVisualSpec`** (**`boundedCompositionCaption`**, **`boundedEmphasisSummaryLine`**) |
...
| `src/scene/boundedSceneSelection.test.ts` | **Vertical Slice v5** — stable **`glass.sel.v0:*`** ids; **`buildBoundedSelectionHitTargetsFromPrimitives`** / **`buildBoundedSelectionHitTargetsForScene`** + **`hitTestBoundedSelection`**; **`unionBoundingRectForSelectionId`**; **`buildBoundedInspectorLines`** (none / region / cluster) |
