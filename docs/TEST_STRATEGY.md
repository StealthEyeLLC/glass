...
| `bridge::tests::ws_live_session` | **`live_session_subscribe`** → `session_hello` + `session_snapshot_replaced` after collector store change; F-IPC multi-connection harness; frozen HTTP snapshot contract regression; **integration tests that open many F-IPC connections must accept in a loop** (not single `accept()`), matching `run_ipc_dev_tcp_listener` |
| `glass_bridge::live_session_ws` (unit) | Provisional `VecDeque` overflow placeholder; **`LIVE_WS_REASON_*` never equals** frozen HTTP **`RESYNC_HINT_REASON_*`** (F-03 separation); see `docs/F03_LIVE_BACKLOG_FREEZE_PROPOSAL.md` |
