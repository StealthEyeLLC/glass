...
| `src/live/liveVisualRenderer.test.ts` | **`paintLiveVisualSurface`**: **`GlassSceneV0`** input; optional **`selection`** (`selectedSelectionId`) forwarded to Canvas/overlay paths; `webGpuBundle === null` → full **Canvas 2D**, WebGPU + text overlay hidden; mocked **`renderLiveVisualWebGpuFrame`**: hybrid when GPU + overlay succeed; GPU frame fails → Canvas-only; GPU OK + overlay **`getContext("2d") === null`** → full Canvas (degraded) |
...
| `src/scene/boundedSceneSelection.test.ts` | **Vertical Slice v5–v6** — stable **`glass.sel.v0:*`** ids; **`buildBoundedSelectionHitTargetsFromPrimitives`** / **`buildBoundedSelectionHitTargetsForScene`** + **`hitTestBoundedSelection`**; **`unionBoundingRectForSelectionId`**; **`buildBoundedInspectorLines`** (none / region / cluster); **v8** overlay lines incl. compare + strip reflow hit rects |
...
| `src/scene/boundedClaims.test.ts` | **Vertical Slice v13–v16** — **`computeBoundedSceneClaims`**, **`resolvePrimaryClaimId`** (episode + cluster), append/replace status, **`buildBoundedClaimReceipt`** (**`glass.receipt.v0`**, ref keys, weakness), **`boundedClaimEvidenceUiLines`**, receipt DOM (**`data-trust-tier`**, **`data-section`**, empty copy, limitation node) |
