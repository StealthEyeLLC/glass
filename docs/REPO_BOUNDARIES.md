...
| `viewer/` | Operator UI, **Tier B static replay** + optional **`?live=1`** **live-session skeleton** (bridge `/ws` + HTTP snapshot consumer — `src/live/`); **not** WebGPU live scene, **not** authoritative ingest | Authoritative session/event truth in packs only; live path is **bounded / replacement-style** per bridge contract |
