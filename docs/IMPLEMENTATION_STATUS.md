...
| Bridge | `glass_bridge::resync` types + **provisional** threshold constant; **`glass_bridge` binary** — loopback HTTP (`127.0.0.1:9781` default) + WS handshake-only; optional **`--collector-ipc-endpoint` / `--collector-ipc-secret`** (loopback TCP) so `GET /sessions/:id/snapshot?cursor=&max_events=` fetches **bounded** events from `glass-collector ipc-serve` (`collector_ipc` metadata on success; **503** `collector_ipc_unavailable` if TCP/handshake/RPC fails). **`snapshot_cursor`** (`v0:empty` \| `v0:off:N` \| `v0:off:0`) is documented as a **bounded prefix / empty-view label**, not a live delta offset; **`bounded_snapshot`** echoes collector `FipcBoundedSnapshotMeta` (origin + truncation). **`resync_hint`** is populated **honestly** when truncation, per-RPC polls, or retained tail replacement imply **no delta continuity** — not a recovery guarantee. **`retained_snapshot_unix_ms`** when the session matches retained-loop meta (telemetry hint; not an SLA). **`live_session_ingest`** remains **false** (no WS delta stream) |
