...
| Collector | **`glass_collector` library**: raw `RawObservation`, adapters (`procfs_process` on Linux), `procfs_session` + `ingest_procfs_raw_to_session_log`, `procfs_ipc_feed` (per-RPC procfs/fixture → normalized JSON), **`procfs_retained_loop`** (optional background poll → **bounded tail** in `SnapshotStore`; `retained_procfs_poll_tick` for deterministic tests), `RetainedPollMeta` + optional F-IPC `retained_snapshot_unix_ms`, `FidelityReport`, `pipeline`, IPC **types** + **provisional dev TCP** F-IPC (`ipc_dev_tcp`, `SnapshotStore`, `IpcDevTcpRuntime`, NDJSON `FipcBridgeToCollector` / `FipcCollectorToBridge`), privilege context. **`glass-collector` binary**: `capabilities`; **`sample-procfs`**; **`normalize-procfs`** → **unsanitized** dev `.glass_pack` (or `--events-json-stdout`); **`export-procfs-pack`** → share-safe pack; **`ipc-serve`** → loopback TCP F-IPC (bounded snapshot RPC; seed JSON; **`--procfs-session`** = per-request repoll for that id; **`--procfs-retained-session`** + interval + max-events = **retained** store updates — mutually exclusive **same** session id with `--procfs-session`; **`live_session_ingest` stays false**) |
