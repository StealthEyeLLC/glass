...
| Bridge | **Bounded-era F-04 frozen** (`docs/PHASE0_FREEZE_TRACKER.md`, `docs/contracts/bridge_session_snapshot_bounded_v0.schema.json`): opaque **`snapshot_cursor`**; **`bounded_snapshot.snapshot_origin`** disambiguates `v0:empty` (unknown vs per-RPC empty poll); **`resync_hint.reason`** tokens = `glass_bridge::resync::RESYNC_HINT_REASON_*`; optional non-normative **`detail`**; missing **`resync_hint`** = no extra bounded warning (not live-safe). `glass_bridge::resync` also exposes **provisional** **`PROVISIONAL_BACKLOG_EVENT_THRESHOLD`** (live backlog — **not** frozen). **`glass_bridge` binary** — loopback HTTP + WS handshake-only; optional **`--collector-ipc-endpoint` / `--collector-ipc-secret`** so `GET /sessions/:id/snapshot?cursor=&max_events=` fetches **bounded** events from `glass-collector ipc-serve` (**503** if F-IPC fails). **`retained_snapshot_unix_ms`** when retained-loop meta applies (telemetry; not an SLA). **`live_session_ingest`** remains **false** |
...
1. Close remaining Phase 0 items (golden method F-01, **F-03 live backlog** policy, sanitization F-05). **Live ingest / WS deltas** when ready — build on frozen bounded contract without breaking `RESYNC_HINT_REASON_*` tokens.
