...
| Bridge | **Bounded-era F-04 frozen** (`docs/PHASE0_FREEZE_TRACKER.md`, `docs/contracts/bridge_session_snapshot_bounded_v0.schema.json`): opaque **`snapshot_cursor`**; **`bounded_snapshot.snapshot_origin`** disambiguates `v0:empty` (unknown vs per-RPC empty poll); **`resync_hint.reason`** tokens = `glass_bridge::resync::RESYNC_HINT_REASON_*`; optional non-normative **`detail`**; missing **`resync_hint`** = no extra bounded warning (not live-safe). **`live_session_delta_skeleton`** (`glass_bridge::live_session_ws`, `docs/contracts/live_session_ws_skeleton_v1.md`): optional **`live_session_subscribe`** over **`GET /ws`** → bounded F-IPC **polling** → `session_snapshot_replaced` when snapshot fingerprint changes — **replacement/bounded** semantics, **not** append-only deltas; live-era **`LIVE_WS_REASON_*`** on WS overload (**not** HTTP frozen tokens). `glass_bridge::resync` also exposes **provisional** **`PROVISIONAL_BACKLOG_EVENT_THRESHOLD`** (F-03 live backlog — **not** frozen). **`glass_bridge` binary** — loopback HTTP + WS; optional **`--collector-ipc-endpoint` / `--collector-ipc-secret`** for snapshots + WS polling. **`retained_snapshot_unix_ms`** when applicable (telemetry; not an SLA). **`live_session_ingest`** remains **false** |
...
- Bridge **final live ingest**: durable outbound delta queue, viewer backpressure policy (**F-03** still open), resync fault-injection harness, **non-polling** transport to collector. Current **`live_session_delta_skeleton`** only **polls** F-IPC and emits **bounded replacement** notices — **not** append-only history
