...
- Bridge **final live ingest** (push-based collector → bridge, durable delta log, live viewer client): **not** in v0; F-IPC remains **provisional TCP**. **`live_session_ws`** now implements **F-03 v0** per-connection queue + backpressure; it still **polls** F-IPC — **not** append-only history
...
**Bounded-era F-04** (snapshot cursor + `resync_hint` on `GET /sessions/:id/snapshot`) is **closed**. **F-03 v0** queue + **`session_delta` v0 wire skeleton** — **`live_session_ws`** (see `docs/F03_LIVE_BACKLOG_FREEZE_PROPOSAL.md`, `docs/contracts/live_session_ws_session_delta_v0.md`). **Still provisional / next:** non-empty **`session_delta.events`** from push ingest, **F-04 live-era** HTTP extensions, F-IPC transport freeze, F-05 sanitization, etc.

## Next engineering steps

1. Close remaining Phase 0 items (golden method F-01, sanitization F-05). **Next live-session engineering:** collector push / populated **`session_delta.events`** when ingest exists; **additive** live-era HTTP / WS fields only — **do not** break frozen bounded HTTP tokens or opaque cursor contract without a version bump.
