...
| Collector | **`glass_collector` library**: raw `RawObservation`, adapters (`procfs_process` on Linux), `procfs_session` + `ingest_procfs_raw_to_session_log`, `FidelityReport`, `pipeline`, IPC **types** + **provisional dev TCP** F-IPC (`ipc_dev_tcp`, `SnapshotStore`, NDJSON `FipcBridgeToCollector` / `FipcCollectorToBridge`), privilege context. **`glass-collector` binary**: `capabilities`; **`sample-procfs`**; **`normalize-procfs`** → **unsanitized** dev `.glass_pack` (or `--events-json-stdout`); **`export-procfs-pack`** → share-safe pack; **`ipc-serve`** → loopback TCP F-IPC server (bounded snapshot RPC; optional `--seed-session` / `--seed-events-json`) |
...
- Bridge **live ingest**: WS delta stream, resync fault-injection harness, wiring snapshot store to **real** collector session timelines (beyond in-memory `SnapshotStore` seed / tests)
...
4. Replace **provisional TCP** F-IPC with human-frozen transport (Unix socket / peer creds); feed `SnapshotStore` from real capture path; honest WS deltas + resync harness (drops/dupes/reorder); live viewer client (Phase 5–6); WebGPU scene replay (Phase 6) — separate track.
