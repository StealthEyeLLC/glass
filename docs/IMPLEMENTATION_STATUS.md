...
| Collector | **`glass_collector` library**: raw `RawObservation`, adapters (`procfs_process` on Linux), `procfs_session` + `ingest_procfs_raw_to_session_log`, `procfs_ipc_feed` (procfs/fixture → normalized JSON for snapshots), `FidelityReport`, `pipeline`, IPC **types** + **provisional dev TCP** F-IPC (`ipc_dev_tcp`, `SnapshotStore`, `IpcDevTcpRuntime`, NDJSON `FipcBridgeToCollector` / `FipcCollectorToBridge`), privilege context. **`glass-collector` binary**: `capabilities`; **`sample-procfs`**; **`normalize-procfs`** → **unsanitized** dev `.glass_pack` (or `--events-json-stdout`); **`export-procfs-pack`** → share-safe pack; **`ipc-serve`** → loopback TCP F-IPC server (bounded snapshot RPC; optional `--seed-session` / `--seed-events-json`; optional **`--procfs-session`** + **`--procfs-from-raw-json`** (non-Linux) or Linux `/proc` poll — **per-request** poll + normalize for matching session id; **`live_session_ingest` stays false**) |
...
| Bridge | `glass_bridge::resync` types + **provisional** threshold constant; **`glass_bridge` binary** — loopback HTTP (`127.0.0.1:9781` default) + WS handshake-only; optional **`--collector-ipc-endpoint` / `--collector-ipc-secret`** (loopback TCP) so `GET /sessions/:id/snapshot` fetches **bounded** events from `glass-collector ipc-serve` (`collector_ipc` metadata on success; **503** `collector_ipc_unavailable` if TCP/handshake/RPC fails). When the collector serves **`--procfs-session`** for that id, the HTTP snapshot reflects **real** procfs→normalize output (still bounded per RPC, not a timeline stream). **`live_session_ingest`** remains **false** (no WS delta stream) |
...
- Bridge **live ingest**: WS delta stream, resync fault-injection harness, incremental cursor semantics beyond provisional `v0:off:n` / `v0:empty` (procfs-backed snapshots are **per-request** polls, not a retained session timeline in the collector)
...
4. Replace **provisional TCP** F-IPC with human-frozen transport (Unix socket / peer creds); optional retained collector session store + honest WS deltas + resync harness (drops/dupes/reorder); live viewer client (Phase 5–6); WebGPU scene replay (Phase 6) — separate track.
